//! `notify` - Cross-platform notification CLI tool built for humans and autonomous AI agents.
...
fn main() -> ExitCode {
    // Error envelope can be rendered before parsing succeeds, so format is checked from raw args.
