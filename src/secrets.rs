//! Where secrets live: the native OS keystore, one entry per profile under `account:{name}`.
//!
//! DPAPI on Windows, the Keychain on macOS, libsecret on Linux. When none is available the tool
//! refuses to start rather than silently writing a file - `NOTIFY_ALLOW_PLAINTEXT_STORE=1` is
...
fn build() -> Result<Store> {
    if let Some(forced) = std::env::var("NOTIFY_SECRET_STORE").ok().filter(|s| !s.trim().is_empty()) {
        return match forced.to_lowercase().as_str() {
            ...
            _ => Err(Error::invalid(format!(
                "NOTIFY_SECRET_STORE='{forced}' is not a backend available on this platform."
            ))
            .fix("Unset NOTIFY_SECRET_STORE, or set it to one of: dpapi, keychain, libsecret, plaintext.")),
        };
    }
    ...
    fallback(
        "libsecret is not installed, so there is no OS keystore to hold your credentials. \
         Install it with: sudo apt install libsecret-tools (or the equivalent for your distro).",
    )
}

fn fallback(reason: &str) -> Result<Store> {
    if std::env::var("NOTIFY_ALLOW_PLAINTEXT_STORE").as_deref() == Ok("1") {
        return Ok(Store::Plaintext);
    }
    Err(Error::other("No secure credential store is available on this machine.")
        .detail(reason)
        .fix("Install a keystore, or set NOTIFY_ALLOW_PLAINTEXT_STORE=1 to store credentials in a 0600 file instead."))
}
...
fn warn_plaintext() {
    ...
    WARNED.get_or_init(|| {
        eprintln!(
            "warning: Secrets are stored unencrypted in {} (NOTIFY_ALLOW_PLAINTEXT_STORE=1).",
...
mod dpapi {
    ...
    pub fn protect(plain: &[u8]) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(Error::other("DPAPI could not encrypt the credentials.")
    ...
    pub fn unprotect(data: &[u8], path: &Path) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(Error::other("The stored credentials could not be decrypted.")
                .detail(format!(
                    "DPAPI failed: {}. This happens when the file was written by a different Windows user or on a different machine.",
                    std::io::Error::last_os_error()
                ))
                .fix(format!("Delete {} and run: notify accounts add <name>", path.display())));
        }
        Ok(take(out))
    }
}
